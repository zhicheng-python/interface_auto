...
do_excel = ReadExcel(sheet_name=do_read_yaml.read_config("excel", "sheet5_name"))
    ...
    def test_invest(self, item):
        ...
        try:
            ...
            if "加标成功" in item.title:
                loan_id = result.json().get('data').get('id')
                setattr(HandleParams, 'loan_id', loan_id)
                # setattr(do_handle_params, 'loan_id', loan_id)
